(ns com.fulcrologic.rad.database-adapters.psql.resolvers
  (:require
    ...
    [com.fulcrologic.rad.database-adapters.psql.query :as sql.query]
    [com.fulcrologic.rad.database-adapters.psql.schema :as sql.schema]
...
(defn form->sql-value [{::attr/keys    [type cardinality]
  ...
  (cond
    ...
    (and
      (or
        (sequential? form-value)
        (set? form-value))
      (every? int? form-value)) (int-array form-value)
    ...
    (doseq [schema (keys connection-pools)]
      (let [ds             (get connection-pools schema)
        ...
        (jdbc/with-transaction [ds ds {:isolation :serializable}]
          ...
          (jdbc/execute! ds ["SET CONSTRAINTS ALL DEFERRED"])
